...
vLLM support is in flight in [PR #41765](https://github.com/vllm-project/vllm/pulls), with the same surface area: `stream_format` request field, server-side `ToolWatcher`, dict-gated zstd, and the `Codec-Zstd-Dict` response header. Watch that PR for status.
