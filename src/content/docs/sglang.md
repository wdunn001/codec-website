...
> **If you just want a working server**, use the pre-built [codec-sglang Docker image](/docs/codec-sglang/), one container, GPU-ready, supervisor and patches already applied. This page is for the DIY path: vanilla upstream sglang plus the Codec patches.
...
> **Why a fork?** sglang upstream merges new transports through formal proposals; while the Codec patches stabilise we ship them in `wdunn001/sglang` (CUDA 12 builds in [`wdunn001/codec-supervisor`](https://github.com/wdunn001/codec-supervisor)). The patch is a thin overlay, cherry-pickable on any recent upstream nightly.
...
This is the source of the tool-call detection speedup measured in [RESULTS.md](https://github.com/wdunn001/Codec/blob/main/packages/bench/RESULTS.md), **26.7&times;** on the v0.4.1 lab box (EPYC 8124P / gcc:13) running the libcodec C99 microbench, with the speedup ratio in ToolWatcher's favour by `~26-100×` depending on host. The server skips its usual "detokenize and regex-match against tool delimiters" step entirely; the client gets pre-segmented frames and can dispatch with no further parsing.
...
`stream_format` is the only Codec-specific request knob. The compression negotiation is the standard HTTP `Accept-Encoding`. `gzip, identity` is the safe default for any streaming workload. gzip preserves TTFT and gives 30-40&times; wire savings on Codec frames.

### zstd, with a dict

`zstd` is supported but **only when the server has a pre-trained dictionary loaded for the request's `stream_format`**. See [Protocol &raquo; Compression](/docs/protocol/#zstd-is-dict-only) for the rule and the rationale. Without a dict, sglang's compression module falls through to gzip even if the client advertised `zstd`. No-dict zstd's bytes match gzip but the buffered middleware adds a 334&times; TTFB cliff at 2K tokens.
...
Deployments that don't load a dict keep getting gzip on every request, backward-compatible by default. Empty registry &rarr; zstd never picked.

## vLLM and llama.cpp

The same surface area (`stream_format` request field, server-side `ToolWatcher`, dict-gated zstd, and the `Codec-Zstd-Dict` response header) is shipped in pre-built form as [`codec-vllm`](/docs/codec-vllm/) and [`codec-llamacpp`](/docs/codec-llamacpp/).

## See also

- [TypeScript](/docs/typescript/), [Python](/docs/python/), [.NET](/docs/dotnet/), client walkthroughs.
- [Tool calling](/docs/tool-calling/), detail on `ToolWatcher` events and dispatch.
