...
|---|---|---|
| **Use when** | you have an existing MCP server | you're building a new tool from scratch |
| **What it does** | wraps `CallToolResult` with per-block `_meta` token IDs | full bolt-on contract (manifest + precache + runtime) |
| **Tokenization** | at MCP-tool runtime against a pinned map | at build time, ship the cache file |
| **Wire** | MCP JSON-RPC + leaf-meta payload | binary bolt-on (gateway ↔ tool over msgpack/protobuf) |
| **Use case** | upgrade existing MCP servers in-place | author Codec-first tools, host independently |
...
2. The gateway's [`ToolWatcher`](/docs/tool-calling/) matches the control tokens (one 32-bit compare per token, **no detokenize**) and routes the argument IDs to your tool.
