...
kind: improvement
version: v0.3.2
summary: Re-ran the full cross-stack matrix after patching two bench-driver bugs (C/TS token-decode fallback, vllm REPS=1 noise). All three engines × six client languages now produce byte-identical Codec frames per cell, including vllm. That one previously read as 0/24 unanimous in the post-mortem.
links:
...
1. C and TS demos were emitting `tokens_emitted=0` for compressed cells (the `tokens` field was only populated on `identity` decode). That threw off the unanimity check.
...
3. JSON-SSE rows have 10-16 B per-client framing-accounting drift that's structural.
