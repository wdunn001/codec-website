...
- **`@codecai/mcp-leaf`**, tool-author SDK. Wrap your `CallToolResult` with `wrapToolCall(result, meta)` and the gateway sees the pre-tokenized IDs and skips the re-tokenize on every request.
