`F6cf`!v0.4.1 (cross-client dict-zstd, llama.cpp br+zstd, synthetic protocol bench)`!`f

`F9992026-05-16 - v0.4.1 - improvement`f
...
├───────────────────────────────────────────────────────┼──────────────────────────────────────┤
│ Cross-stack matrix (24 cells × 6 clients × 3 engines) │ 24/24 wire + 24/24 decode unanimous  │
│                                                       │ on every engine                      │
│ §1 synthetic protocol-only                            │ 4.8× to 391.9× depending on content  │
│                                                       │ compressibility                      │
│ §1b engine-output (content-dependent)                 │ sglang 1,707× / vllm 137× /          │
│                                                       │ llama.cpp fp16 3,868× @ 2K           │
│ Cross-vocab translator (Llama-3 → Qwen-2)             │ 15.1× wire @ 2K, bridge CPU within   │
│                                                       │ noise                                │
...
│ MCP leaf-mode, tool-result-side (tiny)                │ +211 bytes wire (leaf 3× larger on   │
│                                                       │ tiny results) / 12.4× faster         │
│                                                       │ consumer CPU (re-tokenize 0.052 ms → │
│                                                       │ meta-read 0.004 ms); wire crossover  │
│                                                       │ where leaf ≤ plain sits ~300+ chars  │
│                                                       │ per text block                       │
│ ToolWatcher microbench                                │ 481 Mtok/s vs detokenize 18 Mtok/s → │
│                                                       │ 26.7× speedup                        │
└───────────────────────────────────────────────────────┴──────────────────────────────────────┘
...
GitHub Release v0.4.1 (https://github.com/wdunn001/Codec/releases/tag/v0.4.1)

CHANGELOG entry (https://github.com/wdunn001/Codec/blob/main/CHANGELOG.md#v041--2026-05-16)

New synthetic-stream bench (https://github.com/wdunn001/Codec/blob/main/packages/bench/results/2026-05-15T20-00-00Z/synthetic/wire.json)

MATRIX.md, 2026-05-15T20-00-00Z (https://github.com/wdunn001/Codec/blob/main/packages/bench/results/2026-05-15T20-00-00Z/MATRIX.md)
