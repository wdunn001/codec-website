`F6cf`!What is Codec?`!`f

`F999A control-plane primitive for AI inference. Lets gateways, routers, agents, and tool dispatchers operate on raw token IDs end-to-end, with detokenization localized at the edges that need text.`f
...
• `!Wire-native streaming.`! Token IDs flow as length-prefixed binary frames over plain HTTP. Compression layers on top of the same wire. Receipts: a short chat reply ships in 226 B (vs 15.2 KB JSON-SSE = ~67×); a 2 K-token agent stream in 140 B-354 B (engine-dependent), `!1,707×`! on sglang, `!3,868×`! on llama.cpp fp16 with the full msgpack + dict-zstd stack. The honest `*protocol-only`* range across token-distribution profiles is `!4.8×-391.9×`! (v0.4.1 synthetic-stream bench, decoupled from engine + model). TTFB is within 1 ms of the JSON path on the same server. v0.4.1 cross-stack matrix (https://github.com/wdunn001/Codec/blob/main/packages/bench/results/2026-05-15T20-00-00Z/MATRIX.md), 24/24 wire AND 24/24 decode unanimous across 6 clients × 3 engines.
• `!Tool-call dispatch without detokenization.`! 'ToolWatcher' matches reserved control IDs in the raw token stream, one 32-bit compare per token. v0.4.1 lab microbench on EPYC 8124P + gcc:13: `!2.08 ms vs 55.42 ms`! on a 1 M-token stream (`!26.7× faster`! than detokenize+regex; the speedup remains in ToolWatcher's favour by ~26-100× depending on host). The `[MetaMCP gateway`:/page/codecai/docs/codec-metamcp.mu] is the canonical place this primitive lives in production; the same hook works in any inference proxy, agent runtime, or middleware.
...
• `!Inference gateways.`! Token IDs at the wire and at the dispatch layer. ToolWatcher signals fire on raw IDs in real time; the gateway's text-touching code (JSON-RPC dispatch to MCP servers, human-display sinks) runs once at the seam. The `[MetaMCP gateway`:/page/codecai/docs/codec-metamcp.mu] ships this pattern as a docker image; the same primitive is reusable in any proxy, agent runtime, or middleware.
• `!Heterogeneous model meshes.`! 'Translator' carries one model's stream into another's vocabulary without UTF-8 ever crossing the wire. Measured Llama-3 → Qwen-2 handoff: `!15.1× smaller wire`! with bridge CPU within noise of detokenize+retokenize, byte-identical Qwen-2 output. Source: 'packages/bench/results/2026-05-15T20-00-00Z/translator/' (https://github.com/wdunn001/Codec/tree/main/packages/bench/results/2026-05-15T20-00-00Z/translator).
• `!Agent-to-agent traffic.`! No human is reading these tokens. Vocab fixed at handshake, dict pre-shared, msgpack frames collapse to a control byte plus a delta. The `!1,707×`! sglang / `!3,868×`! llama.cpp engine-output headlines at 2 K tokens live here.
...
• `!Human-facing chat UIs.`! ~67× smaller wire on a short reply, `!~1,700×`! on long ones (sglang), TTFB within 1 ms of JSON-SSE. The client decodes once into a string at the edge before render; mobile, edge, and chat-platform-scale traffic all benefit. Bandwidth drops without users seeing anything except faster paint on flaky networks.
...
• `!`['codec-sglang'`:/page/codecai/docs/codec-sglang.mu]`!, full Codec stack (msgpack/protobuf, gzip + brotli + dict-zstd). The `!1,707×`! headline lane at 2 K tokens.
• `!`['codec-vllm'`:/page/codecai/docs/codec-vllm.mu]`!, Codec PR over upstream vllm with dicts pre-baked. `!137×`! at 2 K with dict-zstd live (v0.4.1).
• `!`['codec-llamacpp'`:/page/codecai/docs/codec-llamacpp.mu]`!, llama.cpp built from the fork with the Codec PR + streaming br + zstd middleware (added v0.4.1). `!3,868×`! at 2 K with dict-zstd (fp16); ideal for CPU/edge boxes.

If you'd rather build from source against vanilla upstream, see `[sglang vanilla setup`:/page/codecai/docs/sglang.mu] for the DIY path. The wire is bit-identical between the bundled and DIY paths.
...
Next: pick a runtime in the sidebar, or jump to the `[quickstart`:/page/codecai/docs/quickstart.mu].
