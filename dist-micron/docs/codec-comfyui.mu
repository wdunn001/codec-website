`F6cf`!codec-comfyui (Docker)`!`f

`F999ComfyUI image-generation server with the Codec v0.3 latent transport patch. Streams VAE latents on the wire instead of decoded pixels, 48× smaller, decoder runs at the leaf.`f

`F999`*Server`*`f

-

'codec-comfyui' is a pre-built Docker image of ComfyUI (https://github.com/comfyanonymous/ComfyUI) with the Codec v0.3 latent transport patch applied. Stand it up like any image-gen server, point any Codec-aware client at it, and image generations ship as `!VAE latents`! instead of decoded pixels, same physics as text-token streams in `[codec-sglang`:/page/codecai/docs/codec-sglang.mu] / `[codec-vllm`:/page/codecai/docs/codec-vllm.mu], but for diffusion.
...
├─────────────────┼────────────────────┼────────────────────────┼──────────────────────────────┤
│ 'raw'           │ Pack tensor in     │ 1×                     │ Bit-exact baseline           │
│                 │ row-major order    │                        │                              │
│ 'int8'          │ Per-channel        │ 2× over fp16           │ Default for SD-family images │
│                 │ symmetric int8     │                        │                              │
│ 'int4'          │ Per-channel        │ 4× over fp16           │ Aggressive lossy mode        │
│                 │ symmetric int4     │                        │                              │
│                 │ (packed)           │                        │                              │
│ 'int8-adaptive' │ int8 with          │ ~2×                    │ Heterogeneous frames         │
│                 │ per-keyframe       │                        │                              │
│                 │ scales             │                        │                              │
│ 'int4-adaptive' │ int4 with          │ ~4×                    │ Same use case, more lossy    │
│                 │ per-keyframe       │                        │                              │
│                 │ scales             │                        │                              │
│ 'delta+int8'    │ int8 residual      │ 2× + temporal collapse │ Video only                   │
│                 │ against prior      │                        │                              │
│                 │ keyframe           │                        │                              │
│ 'delta+int4'    │ int4 residual      │ 4× + temporal collapse │ Video, most aggressive       │
│                 │ against prior      │                        │                              │
│                 │ keyframe           │                        │                              │
└─────────────────┴────────────────────┴────────────────────────┴──────────────────────────────┘
...
• `[codec-diffusers`:/page/codecai/docs/codec-diffusers.mu], sister image, also a v0.3 latent server. Doubles as the bench/golden perceptual reference.
• `[codec-metamcp`:/page/codecai/docs/codec-metamcp.mu], gateway in front of latent servers + tool servers.
