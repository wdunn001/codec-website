...
'codec-diffusers' is a pre-built Docker image of the HuggingFace diffusers (https://github.com/huggingface/diffusers) reference path with the Codec v0.3 latent transport patch applied. It exposes the same '/v1/images/generations' and '/v1/videos/generations' endpoints as `[codec-comfyui`:/page/codecai/docs/codec-comfyui.mu], and the wire shape is byte-identical, but it sits on top of 'diffusers' instead of ComfyUI's workflow engine.
...
Same as `[codec-comfyui's section`:/page/codecai/docs/codec-comfyui.mu], where a single 'LatentStreamDecoder' works against either server.
