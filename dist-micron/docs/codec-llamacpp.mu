`F6cf`!codec-llamacpp (Docker)`!`f

`F999Pre-built llama.cpp server with the Codec patches applied and a control plane bolted on, in one GPU container. OpenAI-compatible. Smallest of the three.`f
...
`F999┃ `!GPU prereq:`! NVIDIA Container Toolkit + '--gpus all'. The image is built for compute capability 'sm_86' (RTX 3090); use '--build-arg CUDA_DOCKER_ARCH=<arch>' if you rebuild for a different GPU.`f
...
├─────────────────────────────────┼───────────────────────────────────────┼────────────────────┤
│ 'Owner/Repo-GGUF:filename-glob' │ '-hf Owner/Repo-GGUF:filename-glob'   │ Pull a quantized   │
│                                 │                                       │ GGUF from Hugging  │
│                                 │                                       │ Face directly. The │
│                                 │                                       │ default.           │
│ '/absolute/path/to/file.gguf'   │ '--model /absolute/path/to/file.gguf' │ Bind-mounted local │
│                                 │                                       │ model file.        │
└─────────────────────────────────┴───────────────────────────────────────┴────────────────────┘
...
├───────────────────────┼──────────────────────────────────────────┼───────────────────────────┤
│ 'CODEC_INITIAL_MODEL' │ 'Qwen/Qwen2.5-0.5B-Instruct-GGUF:Q4_K_M' │ Model spec (HF id         │
│                       │                                          │ ':filename-glob' or       │
│                       │                                          │ absolute '.gguf' path).   │
│ 'CODEC_BACKEND_ARGS'  │ '--ctx-size 4096 --gpu-layers 999'       │ Verbatim arguments to     │
│                       │                                          │ 'llama-server'.           │
│                       │                                          │ '--gpu-layers 999'        │
│                       │                                          │ offloads everything to    │
│                       │                                          │ GPU.                      │
│ 'CODEC_PORT'          │ '8080'                                   │ Supervisor port.          │
│ 'HF_TOKEN'            │ `*(unset)`*                                  │ Required only for gated   │
│                       │                                          │ GGUF repos.               │
└───────────────────────┴──────────────────────────────────────────┴───────────────────────────┘

>>>Admin API

Identical to `[codec-sglang`:/page/codecai/docs/codec-sglang.mu].
