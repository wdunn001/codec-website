...
If the vendor publishes their own map under `['/.well-known/codec/'`:/page/codecai/docs/discovery.mu], skip the URL/hash and resolve from '(origin, id)':
...
A normal '/v1/completions' POST. 'stream_format: "msgpack"' lives in the BODY because it piggybacks on OpenAI's request schema; the Codec REQUEST HEADERS are 'Accept-Encoding' (compression menu) and 'Codec-Client-Version' (v0.4 capability advertisement). See `[Protocol » Request vs response`:/page/codecai/docs/protocol.mu] for the full split.
...
For deployments with pre-trained zstd dictionaries (see `[Protocol » Compression`:/page/codecai/docs/protocol.mu]), 'codecai' ships small helpers in 'codecai.compression'. The pattern: load the dict bytes once, advertise 'zstd' in 'Accept-Encoding', validate the server's 'Codec-Zstd-Dict' header before decompressing.
...
Cross-vocab handoff that never goes through UTF-8. See `[Translator`:/page/codecai/docs/translator.mu].
